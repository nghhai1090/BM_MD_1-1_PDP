...
class GeneticService {
    ...
    public int caculateTotalTimeOfThisRoutedebug(int vehicleIndex, ArrayList<TransportNode> route) {
        int totalTime = 0;
        int previous = -1;
        int actual = -1;
        int activeTime = 0;
        ...
        for (int i = 1; i < route.size(); i++) {
            ...
            activeTime = getActiveTime(route, i, node);
            int rideTime = distanceMatrix[previous][actual] / vehicleSpeed;
            System.out.println("ride "+rideTime);
            System.out.println("act "+activeTime);
            int time = Math.max(totalTime + rideTime, activeTime);
            System.out.println("timereach "+time);
            int serviceTime = 0;
            if (i != route.size() - 1) {
                serviceTime = transportsArray[node.getTransportIndex()].getAmount() / vehicleLoadFactor;
            }
            System.out.println("stime "+serviceTime);
            totalTime = time + serviceTime;
            System.out.println("leavet "+totalTime);
        }
        return totalTime;
